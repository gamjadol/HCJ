...
1. 이 저장소를 클론(Clone)하거나 `index.html` 파일을 다운로드합니다.
   ...
   ```
2. `index.html` 파일을 웹 브라우저(Chrome, Edge, Safari 등)로 엽니다.
...
```text
.
├── index.html       # HTML + CSS + JS 통합 싱글 파일 (앱 실행 파일)
├── logo.png         # 상단 헤더 로고 이미지 (선택 사항)
└── README.md        # 프로젝트 안내 문서
```

> 💡 **참고**: `logo.png` 파일이 로컬 디렉토리에 없더라도 대체 이미지(Placeholder)가 자동으로 표시됩니다. 로고 클릭 시 `https://nanalab.kr`로 이동합니다.
...
1. **옵션 설정**:
   - 좌측 설정 카드에서 원하는 **캔버스 비율**을 선택합니다. (커스텀 선택 시 직접 너비/높이 비율 입력)
   - **Padding** 또는 **Crop** 방식을 선택합니다. (Padding 선택 시 여백 색상 지정 가능)
2. **이미지 업로드**:
   - 우측 업로드 영역으로 이미지 파일들을 드래그 앤 드롭하거나 **[컴퓨터에서 찾기]** 버튼을 눌러 선택합니다.
3. **변환 및 다운로드**:
   - 리스트에서 각 파일의 작업 진행률을 확인합니다.
   - 작업 완료 후 개별 **[다운로드]** 버튼을 누르거나, 우측 상단의 **[일괄 다운로드 (.zip)]** 버튼을 클릭하여 한 번에 받아갑니다.

---

## 🔗 관련 링크

- **공식 웹사이트**: [nanalab.kr](https://nanalab.kr)
